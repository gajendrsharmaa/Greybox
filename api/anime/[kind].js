/**
 * Greybox API (Vercel): GET /api/anime/:kind?page=1
 * :kind = series | movies. Upstream: discover/tv | discover/movie.
 */
'use strict';
...
module.exports = async function handler(req, res) {
  try {
    ...
    const raw = req.query && req.query.kind !== undefined ? req.query.kind : '';
    const kind = String(Array.isArray(raw) ? raw[0] : raw || '').toLowerCase();
    const tmdbType = gb.ANIME_KINDS[kind];
    if (!tmdbType) return json(res, 400, { error: 'Unknown anime kind: ' + (kind || '(empty)') + '. Use: series, movies' });

    const page = gb.parsePage(req.query && req.query.page);
    const data = await gb.tmdbGet(creds, 'discover/' + tmdbType, {
      language: 'en-US',
      page,
      with_genres: '16',
    ...
    return json(res, 200, gb.shapeList(data, tmdbType), true);
  } catch (e) {

/**
 * Greybox API (Cloudflare): GET /api/anime/:kind?page=1
 * :kind = series | movies  (Animation genre 16 + Japanese original language).
 * Upstream: discover/tv | discover/movie.
 */
import { ANIME_KINDS, getCreds, parsePage, shapeList, tmdbGet } from '../../lib/greybox.js';
...
export async function onRequest(context) {
  try {
    ...
    const raw = params.kind ?? params['kind'] ?? '';
    const kind = (Array.isArray(raw) ? raw[0] : String(raw || '')).toLowerCase();
    const tmdbType = ANIME_KINDS[kind];
    if (!tmdbType) return json({ error: 'Unknown anime kind: ' + (kind || '(empty)') + '. Use: series, movies' }, 400);

    const url = new URL(request.url);
    const page = parsePage(url.searchParams.get('page'));
    const data = await tmdbGet(creds, 'discover/' + tmdbType, {
      language: 'en-US',
      page,
      with_genres: '16',
    ...
    return cacheable(request, context, shapeList(data, tmdbType));
  } catch (e) {

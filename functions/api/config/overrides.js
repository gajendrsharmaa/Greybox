/**
 * Greybox API (Cloudflare): GET /api/config/overrides
 * Explicit Greybox metadata overrides from D1 — override fields ONLY
 * ({ tmdb_id, media, ...fields }), never TMDB catalog data.
 * Short cache on purpose: edits show up within ~a minute.
 */
import { getDb, readOverrides } from '../../lib/db.js';
...
export async function onRequest(context) {
  try {
    ...
    return json(await readOverrides(db), 200);
  } catch (e) {

/**
 * Greybox API (Cloudflare): GET /api/config/tags
 * Visible tags from D1 with ordered members (identity only: media + TMDB
 * id — no TMDB catalog data here). Hidden tags are excluded entirely:
 * they resolve to no shelf content and no badges anywhere.
 * Short cache on purpose: edits show up within ~a minute.
 */
import { getDb, readPublicTags } from '../../lib/db.js';
...
export async function onRequest(context) {
  try {
    ...
    return json(await readPublicTags(db), 200);
  } catch (e) {
